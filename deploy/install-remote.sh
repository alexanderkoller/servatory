...
(cd "$repo_dir" && cargo zigbuild --locked --release -p servatory-host --target "$rust_target")
binary_path=$repo_dir/target/$rust_target/release/servatory-host
if ! file "$binary_path" | grep -Fq 'statically linked'; then
    echo "Cross-built daemon is not statically linked: $binary_path" >&2
    exit 1
fi

remote_stage=$(ssh -- "$remote_host" 'mktemp -d /tmp/servatory-install.XXXXXX')
if [[ ! "$remote_stage" =~ ^/tmp/servatory-install\.[A-Za-z0-9]+$ ]]; then
...
scp -- "$binary_path" "$script_dir/install-on-host.sh" \
    "$script_dir/99-servatory.rules" "$script_dir/servatory.service" "$script_dir/servatory.yaml" \
